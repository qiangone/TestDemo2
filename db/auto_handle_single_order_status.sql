...
CREATE DEFINER=`root`@`localhost` PROCEDURE `auto_handle_single_order_status`(
confirm_day int, 
appraisal_day int,
finish_day int,
as_handle_day int, 
...
select a.id from order_after_sale a left join order_detail b on a.order_detail_id=b.id
where TIMESTAMPDIFF(DAY  ,a.apply_time,NOW())>=as_handle_day and a.status in(1,2)and b.order_master_id=order_master_id;
...
select a.id,a.order_master_id from order_after_sale a where TIMESTAMPDIFF(DAY  ,a.refund_time,NOW())>=as_finish_day  and a.status=5 and a.order_master_id=order_master_id
union
select a.id,a.order_master_id from order_after_sale a where TIMESTAMPDIFF(DAY  ,a.handle_time,NOW())>=as_finish_day  and a.status=6 and a.order_master_id=order_master_id
;
